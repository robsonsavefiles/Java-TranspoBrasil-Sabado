...
public abstract class ValidadorEntidade<T extends Entidade> extends Validador<T> {
    ...
    public Erro validarId(String conteudo) {
        try {
            ...
            return new Erro(Entidade.CAMPO_ID, "deve ser um número.");
        }
        return null;
    }
    ...
    public Erro validarDataCriacao(T obj) {
        if (obj.getDataCriacao() == null) {
            return new Erro(Entidade.CAMPO_DATA_CRIACAO, "não pode ser nulo.");
        }

        if (obj.getDataCriacao().isAfter(LocalDateTime.now())) {
            return new Erro(Entidade.CAMPO_DATA_CRIACAO, "não pode ser futuro.");
        }

        if (obj.getDataCriacao().isAfter(obj.getDataAlteracao())) {
            return new Erro(Entidade.CAMPO_DATA_CRIACAO, "(" + obj.getDataCriacao() + ") não pode ser maior do que o campo "
    ...
    public Erro validarDataAlteracao(T obj) {
        if (obj.getDataCriacao() == null) {
            return new Erro(Entidade.CAMPO_DATA_ALTERACAO, "não pode ser nulo.");
        }

        if (obj.getDataCriacao().isAfter(LocalDateTime.now())) {
            return new Erro(Entidade.CAMPO_DATA_ALTERACAO, "não pode ser futuro.");
        }

        if (obj.getDataAlteracao().isBefore(obj.getDataCriacao())) {
            return new Erro(Entidade.CAMPO_DATA_ALTERACAO, "não pode ser menor do que o campo " + Entidade.CAMPO_DATA_CRIACAO.getNome() + ".");
        }
