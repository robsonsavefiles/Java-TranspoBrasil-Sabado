...
public class ValidadorVeiculo extends ValidadorEntidade<Veiculo> {
    ...
    public Erro validarAnoFabricacao(String conteudo) {
        ...
        try {
            ...
            return new Erro(Veiculo.CAMPO_ANO_FABRICACAO, "deve ser um número.");
        }

        if (ano > LocalDate.now().getYear()) {
            return new Erro(Veiculo.CAMPO_ANO_FABRICACAO, "não pode ser maior do que o ano atual.");
        }

        if (ano < 1900) {
            return new Erro(Veiculo.CAMPO_ANO_FABRICACAO, "não pode ser menor do que 1900.");
        }
