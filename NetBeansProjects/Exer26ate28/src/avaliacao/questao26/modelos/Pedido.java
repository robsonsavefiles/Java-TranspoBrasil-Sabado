...
public class Pedido extends Entidade {
    ...
    public List<PedidoItem> getItens() {
        ...
        itens.stream().forEach((item) -> {
            retorno.add(new PedidoItem(item.getId(), item.getDataCriacao(), item.getDataAlteracao(), item.getProduto(), item.getQuantidade(),item.getEstoque()));
        });
        return retorno;
    }
    ...
    private double calcularTotal() {
        double soma = 0;
        soma = itens.stream().map((iten) -> iten.getValor()).reduce(soma, (accumulator, _item) -> accumulator + _item);
        return soma;
    }
