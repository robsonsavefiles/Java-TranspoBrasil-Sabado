angular.module("projetos")
        .controller("providenciasListaController", ["$scope", "ProvidenciasService", "ProjetosService", "ChamadosService", "$state",
            function ($scope, ProvidenciasService, ProjetosService, ChamadosService, $state) {
                this.inicializar = function () {
                    $scope.$emit("mudarTituloSubtitulo", "Providências", "Lista");
                ...
                $scope.$on("providenciasServiceRemoverSucesso", function (event, providenciaChamadoRemovida) {
                    var item = null;
                    angular.forEach($scope.providencias, function (providencia) {
                        if (providencia.id === providenciaChamadoRemovida.providencia.id) {
                            item = providencia;
                        }
                    });
